...
export default function ThemenFilter({ issues }: { issues: Issue[] }) {
  ...
  return (
    <>
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-200 mb-8 flex flex-wrap gap-4 items-end">
        ...
        <div>
          ...
          <select value={category} onChange={e => setCategory(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 bg-white">
            {categories.map(c => <option key={c}>{c}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">Status</label>
          <select value={status} onChange={e => setStatus(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 bg-white">
            {statuses.map(s => <option key={s}>{s}</option>)}
          </select>
        </div>
        <button
          onClick={() => { setCategory('Alle'); setStatus('Alle'); setSearch('') }}
          className="text-sm font-medium text-gray-500 hover:text-green-700 active:scale-95 transition-all"
        >
          Zurücksetzen
        </button>
      </div>
      ...
      {filtered.length > 0 ? (
        <>
          ...
          <div className="text-center">
            <button className="border-2 border-green-600 text-green-700 px-8 py-3 rounded-full font-semibold hover:bg-green-50 active:scale-95 transition-all">
